...
delCookie:function(name){ 
    ...
    webupload:function($list,$btn,state,obj,key,cscope,more){
...
"Access-Control-Request-Method": "POST"
    ...
    uploader.on( 'uploadSuccess', function( file,re ) {
        ...
        if(re.code==1){ 
          ...
          obj[key] =re.url;
          more = parseInt(f.getstore('hash')['more'])!=undefined?parseInt(f.getstore('hash')['more']):more;
          key = f.getstore('hash')['key']!=undefined?f.getstore('hash')['key']:key;
          if(more!=1){
             var keyarr = key + 'arr';
            obj[keyarr] = obj[keyarr]||[];
            if (obj[keyarr].length==3) {layer.msg('最多上传三张图片');return;}
            obj[keyarr].push({img:re.url});  
            var imgarr = [];
